...
func (st *Manager) setNextState(ctx context.Context, alertRule *ngModels.AlertRule, result eval.Result, extraLabels data.Labels, logger log.Logger) StateTransition {
	...
	needImage := shouldTakeImage(currentState.State, oldState, currentState.Image, currentState.Resolved)
	if needImage {
	...
	lbls := prometheus.Labels{"needImage": strconv.FormatBool(needImage)}
	st.metrics.StateUpdateDuration.With(lbls).Observe(float64(st.clock.Now().Sub(start).Milliseconds()))
	st.metrics.StateUpdateCount.With(lbls).Inc()
